...
namespace Mn.NewsCms.WebCore.Controllers
{
    public partial class SharedController : BaseController
    {
        ...
        public virtual IActionResult TopMenuMember()
        {
            return PartialView("Navigation/_TopMenuMember");
        }
        [OutputCache(Duration = CmsConfig.Cache3Hour)]
        public virtual IActionResult HeaderMenu()
        {
            var model = new MenuModel();
            model.Menu = _menuBiz.Get(MenuLocation.Top);
            if (model.Menu.EnableCategory && model.Menu.EnableCategory)
        ...
        public virtual IActionResult Footer()
        {
            ...
            {
                var footerMenu = RenderViewToString("Shared.Views.Navigation._FooterMenu", new MenuModel() { Menu = _menuBiz.Get(Constants.Menus.FooterMenuName) });
                model = model.ReplaceAnyCase("[footermenu]", footerMenu);
            }
            if (model.Contains("[socialmenu]"))
            {
                var socialMenu = RenderViewToString("Shared.Views.Navigation._SocialMenu", new MenuModel() { Menu = _menuBiz.Get(Constants.Menus.SocialMenuName) });
                model = model.ReplaceAnyCase("[socialmenu]", socialMenu);
            }

            return PartialView("Shared.Views.Navigation._Footer", model);
        }        

        [OutputCache(Duration = CmsConfig.Cache3Hour)]
        public virtual IActionResult FooterMenu()
        {
            var model = new MenuModel() { Menu = _menuBiz.Get(Constants.Menus.FooterMenuName) };
            return PartialView("Shared.Views.Navigation._FooterMenu", model);
        }
    }
}